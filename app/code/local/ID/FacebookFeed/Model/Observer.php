...
class ID_FacebookFeed_Model_Observer {
  ...
  private function appendXML($p) {
    if( substr( $p['image_link_large'], -4 ) == '.jpg' ) {
      ...
      $product->appendChild ( $this->xml->createElement('g:title', ucwords(strtolower(htmlspecialchars($p['title'])))) );
      ...
      if( $p['gender'] != '' && !in_array($p['gender'], $this->notAllowed) && !in_array($p['gender'], array('Παιδικό - Αγόρι','Παιδικό - Κορίτσι', 'Bebe')) ) {
        if( $p['gender'] == 'Άνδρας' ) {
